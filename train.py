...
class Detr(LightningModule):
    def __init__(self, lr=1e-4, lr_backbone=5e-5, weight_decay=1e-1):  # or 5e-5
        super().__init__()
        self.lr = lr
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("training_loss", loss, batch_size=len(batch), sync_dist=True)
        for k, v in loss_dict.items():
            self.log("train_" + k, v.item(), batch_size=len(batch), sync_dist=True)

        return loss

    def validation_step(self, batch, batch_idx):
        loss, loss_dict = self.common_step(batch, batch_idx)
        self.log("validation_loss", loss, batch_size=len(batch), sync_dist=True)
        for k, v in loss_dict.items():
            self.log("validation_" + k, v.item(), batch_size=len(batch), sync_dist=True)
